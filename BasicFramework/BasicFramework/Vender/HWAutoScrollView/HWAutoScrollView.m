...
- (void)initControlsIsRemoteImage:(BOOL)isRemote
{
    ...
    self.timer = [NSTimer scheduledTimerWithTimeInterval:3 target:self selector:@selector(timingAction) userInfo:nil repeats:YES];
}
...
{
    if (!_scrollView) {
        CGFloat contentWidth = self.imageArray.count * kScreenWidth;
...
- (void)timingAction
{
    CGFloat lastContent = kScreenWidth * (self.imageArray.count - 1);
    ...
    point.x = kScreenWidth * self.pageControl.currentPage;
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    //最后一页图片的x + width
    CGFloat allImageWidth = (self.imageArray.count - 1) * kScreenWidth;
...
- (void)layoutSubviews
{
    ...
    if (self.pageControlPosition == HWPageControlPositionLeft) {//设置分页控制器靠左
        ...
        [self.pageControl mas_makeConstraints:^(MASConstraintMaker *make) {//设置分页控制器居中
